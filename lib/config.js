...
var config = {
    ...
    logTest: function(result, logInfo){
        ...
        assert.match(result.hash, /^0x/, 'log hash should be a hash');
        ...
        assert.match(result.transactionHash, /^0x/, 'transactionHash should be a hash');
        ...
        assert.strictEqual(+result.blockNumber, Number(logInfo.block.blockHeader.number), 'transaction block number should be ' + logInfo.block.blockHeader.number);
        assert.match(result.address, '0x'+ logInfo.tx.to, 'log address should 0x'+ logInfo.tx.to);
