...
var config = {
    ...
    transactionTest: function(result, tx, index, block){
            ...
            assert.strictEqual(result.status, 'mined', 'transaction should have status "mined"');
            ...
            assert.strictEqual(+result.gasLimit, Number(tx.gasLimit), 'transaction gasLimit should be ' + tx.gasLimit);
            ...
            assert.strictEqual(result.data, tx.data, 'transaction data should be ' + tx.data);
            assert.strictEqual(result.to, '0x'+ tx.to, 'transaction to should be 0x'+ tx.to);
            assert.isAddress(Helpers.isAddress(result.from), 'transaction from should be an address');
    }
};
